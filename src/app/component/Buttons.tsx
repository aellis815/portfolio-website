...
export const QualButton: React.FC<QualButtonProps> = ({ active, selectTab, children }) => {
    //setting a default text color dependent on the active state
    const buttonClasses = active ? "text-white" : "text-slate-500";
    //setting the variants for the animation via motion.div
    const variants = {
        default: { width: 0 },
...
export const ProfessionalButton: React.FC<ProfessionalButtonProps> = ({isChecked, handleCheckboxChange}) => {
      ...
      <label className='themeSwitcher shadow-card relative inline-flex cursor-pointer select-none items-center justify-center rounded-md bg-white p-1 m-1'>
        ...
        <span
          className={`flex items-center space-x-[6px] rounded py-2 px-[18px] text-sm font-medium ${
            !isChecked ? 'text-primary bg-[#071952]' : 'text-body-color'
          }`}
        >
          <svg
            width='16'
